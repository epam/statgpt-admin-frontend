...
| `ADMIN_ROLES_CLAIM`  |    No    | The JWT claim name in the access token that contains the user's roles. Must be set together with `ADMIN_ROLES_VALUES`; if only one of the two is provided, the admin check is skipped and all authenticated users are granted access.                                                                                                 | Any string       | `dial_roles`   |
| `ADMIN_ROLES_VALUES` |    No    | Comma-separated list of role values that grant access to the admin portal. Users whose roles claim does not include any of these values will be shown a no-access page. Must be set together with `ADMIN_ROLES_CLAIM`; if only one of the two is provided, the admin check is skipped and all authenticated users are granted access. | Any string       | `admin`        |
